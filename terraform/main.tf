...
resource "aws_security_group" "ec2" {
  ...
  description = "Allow traffic from ALB, monitoring, and SSH"
  vpc_id      = aws_vpc.main.id
...
resource "aws_iam_policy" "s3_backup" {
  ...
  description = "Allow writing to S3 backup bucket"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect = "Allow"
...
resource "aws_security_group" "monitoring" {
  ...
  vpc_id      = aws_vpc.main.id
  description = "Allow Grafana, Prometheus, SSH"
...
# Monitoring Instance (corrected iam_instance_profile as argument)
resource "aws_instance" "monitoring" {
  ...
  user_data = base64encode(file("${path.module}/monitoring_user_data.sh"))
...
resource "aws_iam_policy" "ec2_describe" {
  ...
  description = "Allow describing EC2 instances"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect = "Allow"
